...
- **A mouse that comes back is noticed as soon as you move it, and one that goes away in
 ...
 Going the other way there is no signal at all: a mouse switched off leaves every one of its
 dongle's interfaces in place, so the only way to find out is to ask it. Two things were
 costing time. A failed read waited four seconds for the second read that confirms it, which
 is now half a second: one failure never means offline on its own, since the wireless link
 drops the odd read, but until the second agrees the app still shows the mouse as connected
 with its remaps live. And the app asked the mouse how it was doing every fifteen seconds, so
 switching one off could go unnoticed for that long. While you are using the pointer it now
 asks every four seconds, going back to fifteen once you leave the machine, which is checked
 with a single question to the window server rather than by watching what you do.

 The app also checks the mouse as soon as the Mac wakes, which is where a mouse most often
 disappears.
...
- **MacRazer no longer burns 50-100% of a CPU core while it sits in the menu bar** (#25).
 Since 0.3.0 the app watched the menu bar's light or dark appearance so it could recolour the
 ...
 charging or not. On the machine it was reproduced on, 0.4.1 sat around 50% and peaked over 70%.

 The charging icon now picks its colours each time it's drawn, from the menu bar it's being
 drawn on, so nothing has to watch the appearance at all. Both icons are drawn once and
 reused. The fixed build idles at 0%.

 `MenuBarIconTests` render the real icon under light, dark, vibrant and high-contrast menu
 bars and check its pixels, including one image switching between them. Resolving the colours
 once, the old way, fails both colour tests.
